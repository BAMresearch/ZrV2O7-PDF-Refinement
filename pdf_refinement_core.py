...
"""
================================================================================
  Core Classes for PDF Structural Refinement
================================================================================
...
DESCRIPTION:
This module defines an integrated set of object-oriented classes for performing 
PDF structural refinements. The architectural framework is designed 
to be instantiated and controlled by an external main execution script.

CLASSES:
  - RefinementConfig: Parses and validates project configuration parameters.
  - StructureAnalyzer: Executes geometric calculations on crystal structures.
  - ResultsManager: Manages the generation and documentation of mathematical output files and visualisations.
  - PDFManager: Automates the generation of empirical PDF datasets and formulates associated `PDFContribution` objects.
  - RefinementHelper: Provides static utility functions intended for standardising the internal refinement workflow.
  - PDFRefinement: The primary controller class that orchestrates all other components to execute a structural refinement workflow.
  - PDFWorkflowManager: An extension class that drives refinement operations sequentially across arrays of datasets.
"""
...
# PDFgetX3 requires a speperate installation and license. More information here: 
...
try:
    ...
    print("WARNING: diffpy.pdfgetx not found. Empirical extraction will default to precalculated .gr files.")

    # Create dummy class to prevent ImportErrors during config handling
    class PDFConfig:
...
# =============================================================================
# Import optimisation algorithms from SciPy
...
class RefinementConfig:
    ...
    This class is initialised from a dictionary. It verifies that all required
    parameters are present and dynamically creates output directories
    for storing results.
    """
...
class _ForensicsWrapper:
    """
    Adapts a raw diffpy Structure object to replicate the interface of a 
    refined PDFGenerator phase. This enables the StructureAnalyzer 
    to process raw CIF data without requiring modifications to its source code.
    """
...
class StructureAnalyzer:
    """
    Executes geometric and crystallographic calculations for a defined structure.

    This class computes structural properties such as bond lengths, angles,
    and dihedrals. It also generates mathematical string expressions used to
    apply rigid-body constraints during a refinement procedure.
    """
    ...
    def find_bond_pairs(self, bond_vectors, phase_added_params):
        ...
        Parameters:
        - phase: The current phase of the structure being analysed.
...
class ResultsManager:
    """
    Manages all result-related operations, including plotting, visualisations,
    and saving output components such as summaries, data curves, and CIF files.
    """
    ...
    def visualize_fit_summary(self, cpdf, phase, output_plot_path,
        ...
        """
        visualise and summarize the fit results, including PDF data,
...
class PDFManager:
    """
    Manages the generation and processing of PDF data and empirical contributions.

    This class encapsulates the extraction of experimental data to construct 
    a PDF (G(r)) profile and formulates `PDFContribution` objects from 
    established structural models (CIF files).
    """
    ...
    def generatePDF(self, data_directory, data_filename, composition, qmax, myrange, myrstep, pdfgetx_config):
        ...
        if DIFFPY_AVAILABLE:
            ...
            # Fallback routine to load precalculated .gr
            fallback_dir = getattr(self.config, 'data_dir_gr', 'dataXRDandGofRs/')
            gr_fname = data_filename.replace('.dat', '.gr')
            gr_path = os.path.join(fallback_dir, gr_fname)
            
            if not os.path.exists(gr_path):
                raise FileNotFoundError(f"Diffpy is missing and fallback .gr file not found: {gr_path}")
            ...
            with open(gr_path, 'r') as f:
                for line in f:
                    ...
                    if len(parts) >= 2:
                        try:
                            # Usually diffpy .gr files have r and G(r) in columns 1 and 2
                            data_arr.append([float(parts[0]), float(parts[1])])
                        except ValueError:
                            continue
            
            data_mtx = np.array(data_arr)
            if len(data_mtx) == 0:
                raise ValueError(f"No numerical data extracted from {gr_path}")
            
            raw_r = data_mtx[:, 0]
            raw_g = data_mtx[:, 1]
            
            r0 = np.arange(myrange[0], myrange[1] + myrstep/2, myrstep)
...
class RefinementHelper:
    """
    Provides utility and helper functions utilised throughout the structural 
    refinement workflow. These methods execute common, reusable standardisation tasks.
    """
...
class PDFRefinement:
    """
    Orchestrates the complete PDF refinement workflow. It autonomously manages
    the configuration parameters, dataset processing, structural contributions, 
    recipe objects, and the sequential execution routine.
    """
    ...
    def run_refinement_step(self, i, fitting_range, myrstep, fitting_order, residualEquation, save_every=1):
        ...
        It supports multiple optimisation strategies ('minimize', 'least_squares',
        ...
        def progress_logging_callback(xk):
            """
            Nested callback to log progress at each step of a local optimisation.
        ...
        for step in fitting_order:
            try:
                ...
                if optimizer_choice == 'basinhopping':
                    # Global optimisation: Explores the parameter space broadly.
    ...
    def build_initial_recipe(self):
        ...
        if getattr(self.config, 'use_shape_envelope', True):
    ...
    def _add_instrumental_and_correlation_parameters(self):
        ...
        for phase in self.cpdf._generators:
            ...
            self.fit.restrain(phase_gen.delta2, lb=0.0, ub=10, scaled=True, sig=0.0001)
            ...
            if self.config.refine_qdamp:
                ...
                self.fit.restrain(phase_gen.qdamp, lb=0.0, scaled=True, sig=1e-4)
            ...
            if self.config.refine_qbroad:
                ...
                self.fit.restrain(phase_gen.qbroad, lb=0.0, scaled=True, sig=1e-5)
    ...
    def _add_adp_parameters(self, phase, sgpar):
        ...
        if mode == 'anisotropic':
            ...
            for par in sgpar.adppars:
                ...
                if is_diagonal:
                    self.fit.restrain(par, lb=0.0, scaled=True, sig=0.0005)
                else:
                    self.fit.restrain(par, lb=-0.1, scaled=True, sig=0.0005)
            ...
            for el, info in self.config.detailed_composition.items():
                ...
                self.fit.restrain(element_vars[el], lb=0.0, scaled=True, sig=0.0005)
    ...
    def modify_recipe_spacegroup(self, spacegroup_list, force_cubic_lattice = False):
        """
        ...
        3.  **New Symmetry Application**:
            ...
            - This operation re-analyses the structure to determine the new set
        ...
        Args:
            ...
            force_cubic_lattice (bool): If True, forces the lattice parameters 
                                        to maintain cubic symmetry (a=b=c) 
                                        regardless of the actual space group.
   ...
   """
    ...
    def simulate_pdf_workflow(self, main_config, sim_config):
        ...
        # 2. Inject optimised fit parameters from the sim_config dictionary
...
class PDFWorkflowManager(PDFRefinement):
    """
    Provides an alternative controller framework to manage a sequential series 
    of structural refinements across an extensive list of empirical datasets. 
    It accommodates integrated tracking logs and a systematic checkpoint mechanism.
    """
    def __init__(self, config, pdf_manager, results_manager, helper, analyzer, ncpu, pool):
    ...
    def run_sequential_workflow(self):
        """Executes the automated, multi-dataset refinement workflow.
    
        This method serves as the primary entry point for orchestrating a
        series of refinements. It iterates through the list of datasets
        defined in the project configuration and applies a multi-step
        refinement strategy to each one, as specified in the 'refinement_plan'.
    
        The workflow's behaviour is governed by three main initialization scenarios
        for each dataset, prioritized in the following order:
    
        1.  **Resume from Checkpoint**: If a valid checkpoint file exists for the
            current dataset, the workflow will load the structure and refined
            parameters from that checkpoint and resume the refinement plan from
            the last completed step. This is the highest priority and ensures
            recovery from interruptions.
    
        2.  **Continue from Previous Dataset**: If no checkpoint exists and the
            `start_each_dataset_fresh` configuration flag is False, the workflow
            uses the final, fully refined model from the *previous* dataset as
            the starting point for the current one. This provides continuity
    -            for sequential analyses (e.g., temperature series).
    
        3.  **Start Fresh**: If neither of the above conditions is met (i.e., it is
            the first dataset, or the `start_each_dataset_fresh` flag is set to
            True), the workflow will initialize a new model from scratch. It
            will use the default CIF file specified in the configuration, or
            optionally, a 'special_structure' if one is provided.
    
        The method manages all file I/O, state-tracking between datasets,
        and the execution of the planned refinement steps.
        """
        # Verify that the refinement plan exists in the configuration.
        ...
        # These variables will store the results from the previously completed dataset.
        last_successful_fit = None
        last_successful_cpdf = None
    
        # Begin iterating through each dataset provided in the configuration.
        for idx, dataset in enumerate(self.config.dataset_list):
            ...
            # Process the raw experimental data to generate the PDF (G(r)).
            ...
            start_step = 0
            # Attempt to load a checkpoint for the current dataset.
            ...
            if checkpoint_data:
            ...
            elif not self.config.start_each_dataset_fresh and idx > 0 and last_successful_fit and last_successful_cpdf:
                # SCENARIO B: CONTINUE from the previous dataset's refined model.
                # This is skipped if the 'start_each_dataset_fresh' flag is True.
                ...
                # SCENARIO C: START FRESH from the initial CIF or a special structure.
                # This runs for the first dataset or when forced by the config flag.
                self.log("First dataset or 'start_each_dataset_fresh' is True: Building initial model from scratch.")
                self.cpdf = self.pdf_manager.build_contribution(r, g, cfg, self.config.ciffile, self.config.myrange)
                self.initialize_from_special_structure()
                self.fit = self.build_initial_recipe()
    
            # --- Refinement Loop for the current dataset ---
            # This loop executes the steps defined in the refinement_plan.
            # Track the currently applied space group to avoid redundant recipe rebuilds.
            current_space_group = None
            ...
            for i in range(start_step, len(refinement_plan)):
                ...
                # Modify the model's space group only if it has changed since the last step.
                if 'space_group' in step_params:
                    new_space_group = step_params['space_group']
                    if new_space_group != current_space_group:
                        enforce_cubic = step_params.get('force_cubic_lattice', True)
                        self.modify_recipe_spacegroup(new_space_group, force_cubic_lattice=enforce_cubic)
                        current_space_group = new_space_group
                    else:
                        self.log(f"[INFO] Space group unchanged ({new_space_group}), skipping recipe rebuild.")
                ...
                # Execute the refinement for the current step.
                ...
                # Save progress to a checkpoint file after completing the step.
                self.save_checkpoint(dataset_id, i)
    
            # Finalize and save all results for the current dataset.
            self.results_manager.finalize_results(self.cpdf, self.fit)
    
            # Store the final model and parameters for the next iteration.
