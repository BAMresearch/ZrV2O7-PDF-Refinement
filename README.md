...
The outputs from each refinement run are saved in the following manner:

fits/ └── Project_Name/ └── [timestamp]/ ├── fitting_summary_[step].txt ├── fitting_curve_[step].csv ├── PhaseX_[step].cif ├── [step]summary_plot.pdf ├── PhaseX_partial_fit.png (multi-phase only) ├── PhaseX_partial.csv (multi-phase only) ├── fit_state[step].txt ├── fitting_[step].png ├── _final_extrapolated_fit.png └── _final_extrapolated_fit.csv
